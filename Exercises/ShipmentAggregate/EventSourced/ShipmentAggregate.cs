...
public class ShipmentAggregate
{
    ...
    {
        _projection.Stops = dispatched.Stops.OrderBy(x => x.Sequence)
            .Select(x => new StopState(x.StopId, x.StopType, StopStatus.InTransit))
            .ToList();
    ...
    {
        _projection.CurrentStopState = _projection.Stops
            .SkipWhile(x => x.Status != StopStatus.InTransit)
            .FirstOrDefault()!;
    }
